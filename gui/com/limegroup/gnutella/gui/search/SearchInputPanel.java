...
class SearchInputPanel extends JPanel {
	 ...
	 */
	private HashSet inputPanelKeys = null;
    ...
    private JPanel createBrowseHostPanel() {
        ...
        JTextArea text1 = new JTextArea(GUIMediator.getStringResource("SEARCH_BROWSEHOST_DESC"));
    ...
    private void updateIpText() {
        if(RouterService.acceptedIncomingConnection() &&
           !NetworkUtils.isPrivate()) {
            IP_TEXT.setText(
        			GUIMediator.getStringResource("SEARCH_BROWSEHOST_MYIP")+" \""+
					NetworkUtils.ip2string(RouterService.getAddress())+":"
					+RouterService.getPort()+"\""
			       );
        } else {
            IP_TEXT.setText(GUIMediator.getStringResource("SEARCH_BROWSEHOST_FIREWALLED"));
        }
    }
	...
	private void createInputPanelForNamedMediaType(NamedMediaType nmt) {
		...
		outerPanel.setMaximumSize(new Dimension(paneWidth+40,paneHeight+40));
	...
	private HashSet getInputPanelKeys() {
		if (inputPanelKeys == null) {
			inputPanelKeys = new HashSet();
		}
	    ...
	    public void actionPerformed(ActionEvent event) {
	        if(SCHEMA_BOX.getSelectedSchema() != null) {
				...
				if (!getInputPanelKeys().contains(key)) {
					createInputPanelForNamedMediaType
						(SCHEMA_BOX.getSelectedMedia());
				}
    ...
    private class MoreOptionsListener implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            ...
            JComponent c = (JComponent)((JComponent)getVisibleComponent()).getComponent(0);
            JComponent pane = (JComponent)((JComponent)c.getComponent(0)).getComponent(0);
            if(c instanceof JPanel && pane instanceof JScrollPane) {
            	if(box.isSelected()) {
            		...
            		int paneWidth = (int)pane.getPreferredSize().getWidth();
            		int paneHeight= (int)pane.getPreferredSize().getHeight();
            		c.setMaximumSize(new Dimension(paneWidth+40,paneHeight+40));
            	}
            }
            invalidate();
            revalidate();
